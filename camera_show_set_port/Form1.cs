...
namespace camera_show_set_port
{
    public partial class Form1 : Form
    {
        ...
        private List<string> nameConfigCsv = new List<string>();
        private void GetAllFileConfig() {
            List<string> nameFile = new List<string>();
            try {
                string[] getFile = Directory.GetFiles("../config/");
                nameFile = getFile.ToList<string>();
            } catch { }

            nameConfigCsv = nameFile.FindAll(element => element.Contains("cameraStep_"));
        }
        ...
        private void Form_set_button_save_Click(object sender, EventArgs e)
        {
            ...
            List<string> file = nameConfigCsv.FindAll(element => element.Contains("read2d"));
            if (!form_set_check_read2d.Checked) {
                List<string> fileSup = nameConfigCsv.ToList();

                for (int loop = 0; loop < file.Count; loop++) {
                    fileSup.Remove(file[loop]);
                }

                file = fileSup.ToList();
            }

            Parallel.For(0, file.Count, row => {
                string[] fileLine = File.ReadAllLines(file[row]);

                for (int loop = 0; loop < fileLine.Length; loop++) {
                    if (!fileLine[loop].Contains("Head " + form_set_numer_head.Value + " Port")) {
                        continue;
                    }

                    fileLine[loop] = "Head " + form_set_numer_head.Value + " Port," + form_set_numer_port.Value;
                    File.WriteAllLines(file[row], fileLine);
                    break;
                }
            });
        }
        ...
        private void form_set_numer_head_valuechanged_sup()
        {
            ...
            foreach (string fileSelect in file) {
                string[] fileLine = File.ReadAllLines(fileSelect);
                for (int loop = 0; loop < fileLine.Length; loop++) {
                    if (!fileLine[loop].Contains("Head " + form_set_numer_head.Value + " Port")) {
                        continue;
                    }
                    string[] sup = fileLine[loop].Split(',');
                    form_set_numer_port.Value = Convert.ToInt32(sup[1]);
                    break;
                }
            }
        ...
        private void form_manual_show()
        {
            ...
            g = 0;
            if (g < num) { form_manual.groupBox6.Enabled = true; form_manual.groupBox6.Visible = true; g++; }
            if (g < num) { form_manual.groupBox7.Enabled = true; form_manual.groupBox7.Visible = true; g++; }
            if (g < num) { form_manual.groupBox8.Enabled = true; form_manual.groupBox8.Visible = true; g++; }
            if (g < num) { form_manual.groupBox9.Enabled = true; form_manual.groupBox9.Visible = true; g++; }
            if (g < num) { form_manual.groupBox10.Enabled = true; form_manual.groupBox10.Visible = true; g++; }
